...
const Education = () => {
	return (
		<section>
			<h2 className="text-center mb-10">Education</h2>
			<div className="relative py-4 after:absolute after:top-0 after:bottom-0 medium:after:left-1/2 after:w-1 after:bg-primary after:z-[2] w-fit medium:w-full smaller:max-w-[44em] mx-auto">
				{educationMap.map((edu, idx) => (
					...
					>
						...
						<span className="block mt-1 text-gray-300">{edu.description}</span>
						<span className="block text-gray-300">{edu.time}</span>
					</div>
				))}
			</div>
		</section>
